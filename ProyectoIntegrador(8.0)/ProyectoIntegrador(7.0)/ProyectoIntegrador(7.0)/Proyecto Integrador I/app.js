...
const sections = ['#hero', '#regiones','#marco', '#comparativa', '#metodologia', '#objetivos', '#transformaciones', '#simuladores', '#comentarios']
  ...
  items.forEach((_,i)=> {
    const s = document.createElement('span'); s.className = 'carousel-dot';
    s.addEventListener('click', ()=> { index = i; updateCarousel(); });
    dotsWrap.appendChild(s);
  });
  carousel.appendChild(dotsWrap);

  let auto = setInterval(()=> { index = (index+1) % items.length; updateCarousel(); }, 4200);
  carousel.addEventListener('mouseenter', ()=> clearInterval(auto));
  carousel.addEventListener('mouseleave', ()=> auto = setInterval(()=> { index = (index+1)%items.length; updateCarousel(); }, 4200));
